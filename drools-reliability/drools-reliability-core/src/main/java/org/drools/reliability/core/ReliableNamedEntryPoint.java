...
public class ReliableNamedEntryPoint extends NamedEntryPoint {
    ...
    protected ObjectStore createObjectStore(EntryPointId entryPoint, RuleBaseConfiguration conf, ReteEvaluator reteEvaluator) {
        ...
        return storesOnlyStrategy ?
                SimpleReliableObjectStoreFactory.get().createSimpleReliableObjectStore(StorageManagerFactory.get().getStorageManager().getOrCreateStorageForSession(reteEvaluator, "ep" + getEntryPointId(), Long.class),
                        reteEvaluator.getSessionConfiguration().getPersistedSessionOption()) :
                new FullReliableObjectStore(StorageManagerFactory.get().getStorageManager().getOrCreateStorageForSession(reteEvaluator, "ep" + getEntryPointId(), Long.class));
    }
